...
(function (factory) {
    ...
    tFooter = '<div class="file-thumbnail-footer">\n' +
        ...
        '   <div class="add-link">\n'+
        '       <div class="textarea" contenteditable="true" maxlength="120"'+ STYLE_SETTING +'>{text}</div>\n'+
        '       <div class="do-some"><span class="pull-left"><i class="fa fa-chain add-address" title="添加超链接"></i> <i class="fa fa-chain-broken delete-address" title="取消超链接"></i></span><span class="pull-right"><i>{textnumber}</i>/120</span></div>\n'+
        '       <div class="address-modal">\n'+
        '           <p class="h5">超链接</p>\n'+
        '           <p>文本内容：<input class="selected-text" type="text" disabled /></p>\n'+
        '           <p>链接地址：<input class="selected-address" type="text" /></p>\n'+
        '           <p>设置标题：<input class="selected-title" type="text" /></p>\n'+
        '           <p>是否在新窗口打开<input class="selected-checkbox" type="checkbox" /></p>\n'+
        '           <div><div class="btn btn-info btn-sm btn-ok pull-left">确定</div><div class="btn btn-sm btn-default pull-right btn-close">取消</div></div>\n'+
        '       </div>\n'+
        '   </div>'+
        '    {progress} {actions}\n' +
        '</div>';
    ...
    FileInput.prototype = {
        ...
        textMore: function(event){// 文字超出提示
            var self = this,$textNum,textLen;
            $textNum = $(event.target).parent('.add-link').find('.do-some').find('.pull-right').find('i');
            textLen = $(event.target).html().replace(/\<[^\>]*>/g,'').replace(/^\s+|\s+$/g, '');

            if(textLen.length<=121){
                $textNum.html(textLen.length);
            }else{
                $(event.target).html($(event.target).html().substring(0,120));
                $textNum.html(120);
                alert('您输入的文字数超出120字符')
            }
            if($(event.target).html().length>0){
                self.selRange = window.getSelection().getRangeAt(0);
                self.selObj = window.getSelection?window.getSelection().toString():document.selection.createRange().text;
            }
            // console.log(self.selRange.startContainer.parentElement);
        },
        addressWord:function(event){//新增 点击添加超链接
            var self = this,
                $textarea = $(event.target).parent().parent('.do-some').siblings('.textarea'),
                $addressModal = $(event.target).parent().parent('.do-some').siblings('.address-modal'),
                $selectedText = $addressModal.find('.selected-text'),
                $selectedAddress = $addressModal.find('.selected-address'), //新增
                $selectedTitle = $addressModal.find('.selected-title'), //新增
                $selectedCheckbox = $addressModal.find('.selected-checkbox'), //新增
                $btnOk = $addressModal.find('.btn-ok'),
                $btnClose = $addressModal.find('.btn-close'),
                $b='',
                $a=$textarea.html();


            if(self.selObj && self.selRange.startContainer.parentElement.tagName=='A'){
                console.log('111')
                var alltext = $(self.selRange.startContainer.parentElement).html(),
                    address = $(self.selRange.startContainer.parentElement).attr('href'),
                    addressTitlt = $(self.selRange.startContainer.parentElement).attr('title'),
                    newWindow = $(self.selRange.startContainer.parentElement).attr('target') == '_blank'?true:false;

                $selectedText.val(alltext);
                $selectedAddress.val(address);
                $selectedTitle.val(addressTitlt);
                $selectedCheckbox.prop('checked',newWindow)
                $addressModal.show();

                var newDOM = document.createElement('DIV'),cloneNode = $(self.selRange.startContainer.parentElement).clone();
                
                $(newDOM).append(cloneNode);
                
                self.currentEl = $(newDOM).html();
                
                // console.log(self.currentEl);

            }else{
                // console.log('222')
                $selectedText.val(self.selObj);
                $addressModal.show();
                $selectedAddress.val('');
                $selectedTitle.val('');
                $selectedCheckbox.prop('checked',false);
            }
            handler(self.$container.find('.btn-ok'),'click',$.proxy(self.btnOk,self))//新增 点击确定超链接
            handler(self.$container.find('.btn-close'),'click',$.proxy(self.btnClose,self))//新增 点击取消超链接
        },
        btnOk:function(event){
            var self = this;
            var $addressModal = $(event.target).parent().parent(),
                $selectedText = $addressModal.find('.selected-text'),
                $selectedAddress = $addressModal.find('.selected-address'),
                $selectedTitle = $addressModal.find('.selected-title'),
                $selectedCheckbox = $addressModal.find('.selected-checkbox'),
                $textarea = $addressModal.siblings('.textarea'),
                $b = '',
                $a = $textarea.html();
            ...
            if(self.currentEl){
                if($selectedAddress.val()){
                    ...
                    var pattern = new RegExp(self.currentEl,'g');
                    $b = $a.replace(pattern,'<a href="'+$selectedAddress.val()+'" title="'+$selectedTitle.val()+'" target="'+target+'">'+$selectedText.val()+'</a>');
                }
            }else{
                if($selectedAddress.val()){
                    if($selectedCheckbox.prop("checked")){
                    ...
                    if($selectedAddress.val()){
                        var selText = $selectedText.val();
                        var pattern = new RegExp(selText,'g');
                        $b = $a.replace(pattern,'<a href="'+$selectedAddress.val()+'" title="'+$selectedTitle.val()+'" target="'+target+'">'+$selectedText.val()+'</a>');
                    }
                }
            }
            ...
            $addressModal.hide();
            self.currentEl='';
            $textarea.find('a').each(function(index, el) {
                if($(el).html()==''){
                    $(el).remove();
                }
            });
        },
        btnClose:function(event){//新增
            var $addressModal = $(event.target).parent().parent(),
                $selectedAddress = $addressModal.find('.selected-address'),
                $selectedTitle = $addressModal.find('.selected-title'),
                $selectedCheckbox = $addressModal.find('.selected-checkbox');
        ...
        delAddressWord:function(event){//新增
            var self = this,$textarea = $(event.target).parent().parent().siblings('.textarea');
            if(self.selRange.startContainer.parentElement.tagName=='A'){
                $(self.selRange.startContainer.parentElement).after($(self.selRange.startContainer.parentElement).html()).remove();
            }
        },
        listen: function () {
            ...
            handler(self.$container.find('.textarea'),'mouseup keyup',$.proxy(self.textMore,self))//新增 文字超出提示
            handler(self.$container.find('.add-address'),'click',$.proxy(self.addressWord,self))//新增 点击添加超链接
            handler(self.$container.find('.delete-address'),'click',$.proxy(self.delAddressWord,self))//新增 点击取消超链接
            handler(self.$container.find('.btn-ok'),'click',$.proxy(self.btnOk,self))//新增 点击确定超链接
            handler(self.$container.find('.btn-close'),'click',$.proxy(self.btnClose,self))//新增 点击取消超链接
        ...
        initPreview: function (isInit) {
            ...
            for(var i=0;i<self.initialPreviewConfig.length;i++){//新增 反绑数据
                if(self.initialPreviewConfig[i].text){
                    var len = self.initialPreviewConfig[i].text.replace(/\<[^\>]*>/g,'').replace(/^\s+|\s+$/g, '').length;
                    out.content = out.content.replace('{text}',self.initialPreviewConfig[i].text).replace('{textnumber}',len);
                }else{
                    out.content = out.content.replace('{text}','').replace('{textnumber}','0');
                }
            }
        ...
        previewFile: function (i, file, theFile, previewId, data) {
            ...
            if (chkTypes || chkMimes) {
                ...
                content = content.replace(/\{text}/g,'').replace(/\{textnumber}/g,'0');//新增
                self.clearDefaultPreview();
                self.$preview.append("\n" + content);
        ...
        validateImage: function (i, previewId, fname, ftype) {
            ...
            handler($thumb.find('.textarea'),'mouseup keyup',$.proxy(self.textMore,self));//新增 划词计数
            handler($thumb.find('.add-address'),'click',$.proxy(self.addressWord,self));//新增 点击添加超链接
            handler($thumb.find('.delete-address'),'click',$.proxy(self.delAddressWord,self));//新增 点击取消超链接
